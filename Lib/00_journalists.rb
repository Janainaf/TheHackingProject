...
puts "Il y a #{journalists.size} handle dans cette array."

#Question 2
puts " Quel est le handle le plus court de cette liste ?  "
shortest = journalists.sort_by(&:length)[0]
puts " Le handle le plus court de cette liste est #{shortest}."
...
puts "Il y a #{count} handle contenant 5 caractères."


#Question 4 
puts "Combien commencent par une majuscule?"
maj_list = journalists.select { |handle| handle[1] == handle[1].upcase }
  puts "il y a #{maj_list.length} qui commencent par une majuscule"
# It works when run separately but seems to be a bug when run with the codes above
...
journalists_sans = []
journalists.each do |handle|
    journalists_sans.push(handle.gsub(/[@_]/, "").capitalize)
end
puts journalists_sans.sort


#Question 6
puts " Voici la la liste de handle par taille des handle (les plus petits en premiers, les plus grands après) #{ journalists.sort_by(&:length)} "
	
#Question 7
puts " Quelle est la position dans l'array de la personne @epenser ? "
puts " La position de @epenser  est la #{journalists.index("@epenser")}"

#Question 8
puts " Une répartition des handle par taille #{ journalists.group_by(&:length) } "
# Actually, it is not in the order asked, but instead grouped by number of elements in the group 
